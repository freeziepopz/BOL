...
local myth = {
	name = "MythikDravenRework", -- script name
	ver = ver, -- script version
	foes = GetEnemyHeroes(), -- enemy champs
	pred = {"VPred", "DivinePred" }, -- prediction table
	modules = {"VPrediction", "DivinePred" }, --libs to load
...
function myth:printChat(msg) -- chat message with prefix
	...
	print("<font color='#D40000'>Welcome to the Mythik Draven Rework! Please report any bugs!</font>")
end
...
local function menu()
	settings = scriptConfig("["..myth.name.."]", "MythikRework")

	TargetSelector.name = "Target Selector"
	...
	settings.ks:addParam("e", "Kill steal with E", SCRIPT_PARAM_ONOFF, true)
	settings.ks:addParam("r", "Kill steal with R", SCRIPT_PARAM_ONOFF, true)

	settings:addSubMenu("Farm", "farm")
	settings.farm:addParam("key", "Farm Key", SCRIPT_PARAM_ONKEYDOWN, false, 86)
	settings.farm:addParam("q", "Farm with Q", SCRIPT_PARAM_ONOFF, true)
...
function OnLoad()
	...
	DelayAction(loadPred, 2) -- load prediction
...
if os.clock() - lastPotion < 15 then return end
	for SLOT = ITEM_1, ITEM_6 do
		...
		if myHero:GetSpellData(SLOT).name == "RegenerationPotion" then
			-- Conditions
			if myHero:CanUseSpell(SLOT) == READY and myHero.health/myHero.maxHealth < 60/100 then
				-- Cast
				CastSpell(SLOT)	
				lastPotion = os.clock()				
			end
